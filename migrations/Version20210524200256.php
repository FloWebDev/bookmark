 ...
 */
final class Version20210524200256 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__app_user AS SELECT id, username, password, email, role, created_at, wallpaper, connected_at, slug FROM app_user');
        $this->addSql('DROP TABLE app_user');
        $this->addSql('CREATE TABLE app_user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(180) NOT NULL COLLATE BINARY, password VARCHAR(255) NOT NULL COLLATE BINARY, email VARCHAR(256) NOT NULL COLLATE BINARY, role VARCHAR(32) NOT NULL COLLATE BINARY, created_at DATETIME NOT NULL, wallpaper VARCHAR(64) DEFAULT NULL COLLATE BINARY, connected_at DATETIME DEFAULT NULL, slug VARCHAR(32) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO app_user (id, username, password, email, role, created_at, wallpaper, connected_at, slug) SELECT id, username, password, email, role, created_at, wallpaper, connected_at, slug FROM __temp__app_user');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__app_user AS SELECT id, username, password, email, slug, role, created_at, connected_at, wallpaper FROM "app_user"');
        $this->addSql('DROP TABLE "app_user"');
        $this->addSql('CREATE TABLE "app_user" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(180) NOT NULL, password VARCHAR(255) NOT NULL, email VARCHAR(256) NOT NULL, slug VARCHAR(32) NOT NULL, role VARCHAR(32) NOT NULL, created_at DATETIME NOT NULL, connected_at DATETIME DEFAULT NULL, wallpaper VARCHAR(64) DEFAULT NULL)');
        $this->addSql('INSERT INTO "app_user" (id, username, password, email, slug, role, created_at, connected_at, wallpaper) SELECT id, username, password, email, slug, role, created_at, connected_at, wallpaper FROM __temp__app_user');
        $this->addSql('DROP TABLE __temp__app_user');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_88BDF3E9F85E0677 ON "app_user" (username)');
        $this->addSql('CREATE UNIQUE INDEX UNIQ_88BDF3E9E7927C74 ON "app_user" (email)');
        $this->addSql('DROP INDEX IDX_1F1B251ED4619D1A');
